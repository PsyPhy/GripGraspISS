...
static bool MainLoop( bool retry, OGL *platform )
{
    ...
    if ( !OVR_SUCCESS( result) ) return retry;
...
Done:
    ...
    return retry || ( result == ovrError_DisplayLost ) || OVR_SUCCESS( result );
...
int WINAPI WinMain(HINSTANCE hinst, HINSTANCE, LPSTR, int)
{
    ...
    // Call Main Loop with retryCreate = false says that the main loop should
	// return an error even if it is an error that could be overcome by retrying. 
	// We will eject with an error message if the main loop fails the first time.
	// Pass a pointer to this to give access to the HandleMessages() method and other parameters.
    VALIDATE( MainLoop( false, &Platform ), "An error occurred setting up the GL graphics window.");

	// Run the main loop. 
    // Passing retry = true says that if the main loop fails, but it is worth trying
	//  again, it should return true. But if it fails or we should quit, then return false.
    while ( MainLoop( true, &Platform ) )
    {
		// Be sure to handle any messages while waiting for the main loop to run again.
        if ( !Platform.HandleMessages() ) break;
