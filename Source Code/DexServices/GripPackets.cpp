...
short extract_reversed_short( const char *bytes ) {
	union {
		short			value;
		unsigned char	byte[2];
	} out;
...
long extract_reversed_long( const char *bytes ) {
	union {
		long			value;
		unsigned char	byte[4];
	} out;
...
float extract_reversed_float( const char *bytes ) {
	union {
		float			value;
		unsigned char	byte[4];
	} out;
