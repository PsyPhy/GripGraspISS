...
// In this version the RTnet server is polled as needed to get the most recent
// data.
...
#include "CodaRTnetPolledTracker.h"

using namespace PsyPhy;

void CodaRTnetPolledTracker::Initialize( const char *ini_filename ) {
...
void CodaRTnetPolledTracker::StartContinuousAcquisition( void ) {
...
void CodaRTnetPolledTracker::RestartContinuousAcquisition( void ) {
	Timer timer;
	TimerStart( timer );
	cl.stopAcq();
	cl.prepareForAcq();
	...
	fOutputDebugString( "CodaRTnetPolledTracker:Restarting acquisition took %f s.\n", elapsed );
	TimerSet( runningTimer, maxContinuous );
}
...
// Does not care if it was actually acquiring or not.
// Does not retrieve the data.
void CodaRTnetPolledTracker::StopContinuousAcquisition( void ) {
	OutputDebugString( "Stopping acquisition ..." );
	cl.stopAcq();
	OutputDebugString( "OK.\n" );
}

void CodaRTnetPolledTracker::Quit( void ) {
...
void CodaRTnetPolledTracker::StartAcquisition( float duration ) {
...
void CodaRTnetPolledTracker::StopAcquisition( void ) {
	acquiring = false;
	for ( int unit = 0; unit < nUnits; unit++ ) {
...
bool CodaRTnetPolledTracker::GetAcquisitionState( void ) {
	Update();
	return( acquiring );
}

int CodaRTnetPolledTracker::Update( void ) {
...
bool CodaRTnetPolledTracker::GetCurrentMarkerFrameUnit( MarkerFrame &frame, int selected_unit ) {
