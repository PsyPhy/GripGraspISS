...
static double ogl_font_height = 12;
static double ogl_font_width = 8;
...
void	OglCircle	(Display display, float x, float y, float radius ) {
  ...
  register  OglParams	*params = (OglParams *) display->parameters;
  float     rx, ry, angle, angle_step = (float)(2.0 * Pi / (double) OGL_MAX_POLY_POINTS);
...

{
  ...
  register  OglParams	*params = (OglParams *)display->parameters;
  float     rx, ry, angle, angle_step = (float) ( Pi / 10.0 );
