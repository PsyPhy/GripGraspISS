		...
		while ( i <= end ) {
			...
			while ( i <= end && distance <= dash_size ) {
				...
				if ( distance + d <= dash_size ) {
					...
					x3 = x1 + (float)(dx * desired / d);
					y3 = y1 + (float)(dy * desired / d);
...
void ViewAutoScaleShorts ( View view, short *array, 
	int start, int end, unsigned size ) {
		...
		else min = (short) view->user_bottom;
		if ( view->user_top < -32000 ) max = -32000; 
		else max = (short) view->user_top;
...
void ViewAutoScaleFloats ( View view, float *array, 
	int start, int end, unsigned size ) {
		...
		pt = (float *)(((char *) array) + start * size);
		min = (float) view->user_bottom;
		max = (float) view->user_top;
...
void ViewScatterPlotAvailableInts (View view, int symbol,
	int *xarray, int *yarray, 
	int start, int end, int step,
	unsigned xsize, unsigned ysize,
	int NA )
	...
	double NA )
{
	...
	for (i = start; i <= end; i += step ) {
		...
		xpt = (double *)(((char *) xarray) + i * xsize);
		ypt = (double *)(((char *) yarray) + i * ysize);
		if ( *xpt != NA && *ypt != NA && *xpt >= view->user_left && *xpt <= view->user_right && *ypt >= view->user_bottom && *ypt <= view->user_top ) {
			ViewSymbol(view, (double) *xpt, (double) *ypt, symbol);
		}
...
void ViewTiltPlotAvailableDoubles (View view, 
	double *x_array, double *tilt_array, 
	int start, int end, int step,
	unsigned xsize, unsigned ysize,
	double NA )
{
	...
	for (i = start; i <= end; i += step ) {
		...
		xpt = (double *)(((char *) x_array) + i * xsize);
		tilt = (double *)(((char *) tilt_array) + i * ysize);
		if ( *xpt != NA && *tilt != NA ) {
			x = UserToDisplayX( view, *xpt );
			r = (float) DegreesToRadians( *tilt );
			sr = radius * (float) sin( r );
			cr = radius * (float) cos( r );
			x1 = x - sr;
			x2 = x + sr;
			y1 = mid - cr;
			y2 = mid + cr;
			Line( view->display, x1, y1, x2, y2 );
