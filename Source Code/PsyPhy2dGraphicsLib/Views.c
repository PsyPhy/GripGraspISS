...
View ViewDisplayPointInside( View view, float x, float y ) {
...
void ViewSetDisplayEdges (View view, float left, float bottom, float right, float top)
...
void ViewShiftRelative ( View view, double x, double y ) {
	...
	float right = (float) x * ( view->display->left - view->display->right );
	float up    = (float) y * ( view->display->top - view->display->bottom );
...
void ViewSetDisplayEdgesRelative (View view, 
	...
	border = view->display->right > view->display->left ? 1.0f : -1.0f; 
	view->display_left = view->display->left + border +
		(float) left * (view->display->right - view->display->left);
	view->display_right = view->display->left - border +
		(float) right * (view->display->right - view->display->left);
	
	border = view->display->bottom > view->display->top ? 1.0f : -1.0f;
	view->display_top = view->display->top + border +
		(1.0f - (float) top) * (view->display->bottom - view->display->top);
	view->display_bottom = view->display->top - border +
		(1.0f - (float) bottom) * (view->display->bottom - view->display->top);
...
void ViewMakeSquare ( View view ) {
	...
	float	center, width, height, new_width, new_height;
	
	if (fabs(view->x_factor) > fabs(view->y_factor)) {
		
		center = (view->display_left + view->display_right) / 2.0f;
		width =  (float) fabs( view->display_right - view->display_left );
		new_width = (float) fabs( view->y_factor / view->x_factor * width ); 
		
		if ( view->display->right > view->display->left )
			ViewSetDisplayEdges( view, center - new_width / 2.0f, view->display_bottom,
			center + new_width / 2.0f, view->display_top );
		else
			ViewSetDisplayEdges( view, center + new_width / 2.0f, view->display_bottom,
			center - new_width / 2.0f, view->display_top );
		...
		center = ( view->display_bottom + view->display_top ) / 2.0f;
		height = (float) fabs( view->display_bottom - view->display_top );
		new_height = (float) fabs(view->x_factor / view->y_factor * height);
		
		if (view->display->bottom > view->display->top)
			ViewSetDisplayEdges(view, 
			view->display_left, center + new_height / 2.0f,
			view->display_right, center - new_height / 2.0f );
		else
			ViewSetDisplayEdges(view, 
			view->display_left, center - new_height / 2.0f,
			view->display_right, center + new_height / 2.0f );
...
void ViewTitle (View view, char *string, float x, float y, double dir) {
...
A value less than 0 for x or y implies a keyword.
    */
	...
	case (OUTSIDE_TOP):
		y = view->display_top -
			view->y_sense * ( 0.3f * TextHeight(view->display, string) + 1 );
		break;
...
void ViewFilledCircle (View view, double x, double y, double radius ) {
	...
	FilledCircle(view->display,
	       (float) UserToDisplayX(view, x), 
		   (float) UserToDisplayY(view, y),
		   (float) fabs( UserToDisplayX(view, radius ) - UserToDisplayX(view, 0.0) ));
...
void ViewSetSpectrumColor( View view, double value ) {
	...
	ColorRGB( view->display, (float) r, (float) g, (float) b );
...
void ViewHorizontalBand( View view, double lower, double upper ) {
	FilledRectangle( view->display, view->display_left + 1, UserToDisplayY( view, lower ), view->display_right - 1, UserToDisplayY( view, upper ) );
}
