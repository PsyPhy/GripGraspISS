...
void  DisplayErase( Display display ) {
...
void  DisplayColor( Display display, int color ) {
  (*(display->set_color))( display, color );
}

void  DisplayColorRGB( Display display, float r, float g, float b ) {
  (*(display->set_color_rgb))( display, r, g, b );
}
...
void DisplayLabel (Display display, char *string, 
  ...
  x_sense = display->right > display->left ? 1.0f : -1.0f;
  y_sense = display->bottom > display->top ? 1.0f : -1.0f;
  ...
  case (RIGHT_JUSTIFY):
    x_offset = - x_sense * (float) TextWidth(display, string);
    right_justify = TRUE;
    break;
    
  case (CENTER_JUSTIFY):
    x_offset = - x_sense * (float) TextWidth(display, string) / 2.0f;
    center_justify = TRUE;
    break;
  ...
  case (TOP_JUSTIFY):
    y_offset = y_sense * (float) TextHeight(display, string);
    break;
    
  case (CENTER_JUSTIFY):
    y_offset = y_sense * lines * TextHeight(display, string) / 2.0f;
    break;
  ...
  while ( *end ) {
    if ( *end == '\n' ) {
      ...
      else if ( center_justify ) x_offset = ( - x_sense * TextWidth( display, start )) / 2.0f;
      else x_offset = 0;
      *end = 0;
...
void DisplayTitle (Display display, char *string, float x, float y, double dir) {
  ...
  */
  ...
  while ( *end ) {
    if ( *end == '\n' ) {
      if ( right_justify ) x_offset = - x_sense * TextWidth( display, start );
      else if ( center_justify ) x_offset = ( - x_sense * (float) TextWidth( display, start )) / 2.0f;
      else x_offset = 0;
      *end = 0;
...
void DisplayArrow (Display display, 
  ...
  float delta_x, delta_y, x, y, dx, dy, orientation, s, c, size;
  ...
  orientation = (float) atan2( delta_y, delta_x );
  s = (float) sin( orientation );
  c = (float) cos( orientation );
...
/*
	...
	The following block of code implements a cache for a Display.
	It allows the screen to be redrawn by storing the graphics atomic
	actions that happen at the level of the Display, and then replaying
	them as needed.

	Originally I implemented this as a linked list, with memory being
	malloced as each graphs atomic action was recorded. But emptying
	the cache became very long when there are a lot of points on the screen.
	So I reimplemented it by allocating large arrays of items at a time
	and then keeping track of how many I have. Instead of allocating memory
	for each item, I take the next one from the array. But I still leave the 
	implementation as a linked list, just to save some coding time.
...
#define CACHE_BLOCK	( 1024 * 1024 * sizeof( DisplayCacheItem ) )
...
DisplayCacheItem *DisplayInsertCacheItem( Display display ) {
	...
	display->cache_last_item++;
	if ( display->cache_last_item >= display->cache_size ) {
		display->cache = (DisplayCacheItem *) realloc( display->cache, CACHE_BLOCK  );
		if ( display->cache == 0 ) {
			MessageBox( NULL, "Memory allocation error.", "PsyPhy2DGrahics", MB_OK );
			exit( -1 );
		}
		display->cache_size += CACHE_BLOCK;
	}
	item = &display->cache[display->cache_last_item]; 
...
void DisplayInitCache( Display display ) {
	...
	// Make sure that memory has been allocated for the cache.
	if ( !display->cache ) {
		display->cache = (DisplayCacheItem *) malloc( CACHE_BLOCK );
		if ( display->cache == 0 ) {
			MessageBox( NULL, "Memory allocation error.", "PsyPhy2DGrahics", MB_OK );
			exit( -1 );
		}
	...
	display->cache_last_item = 0;
	display->last_cache = display->cache = &display->cache[display->cache_last_item];
