...
void LayoutTitle (Layout layout, char *string, float x, float y, double dir) {
  ...
  x_sense = layout->display_right > layout->display_left ? 1.f : -1.f;
  y_sense = layout->display_bottom > layout->display_top ? 1.f : -1.f;
  ...
  */
  ...
  while ( line ) {
    ...
    else if ( center_justify ) x_offset = ( - x_sense * TextWidth( display, line )) / 2.0f;
    else x_offset = 0;
